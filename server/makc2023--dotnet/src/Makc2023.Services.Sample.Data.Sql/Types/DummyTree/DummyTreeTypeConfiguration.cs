...
/// <summary>
/// Конфигурация типа "Фиктивное дерево".
/// </summary>
public class DummyTreeTypeConfiguration : TypeConfiguration<DummyTreeEntity>
{
    #region Constructors

    /// <inheritdoc/>
    public DummyTreeTypeConfiguration(TypesOptions typesOptions)
        : base(typesOptions)
    {
    }
    ...
    /// <inheritdoc/>
    public sealed override void Configure(EntityTypeBuilder<DummyTreeEntity> builder)
    {
        var options = TypesOptions.DummyTree;

        if (options is null)
        {
            throw new NullVariableException<DummyTreeTypeConfiguration>(nameof(options));
        }

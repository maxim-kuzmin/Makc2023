...
/// <summary>
/// Конфигурация типа "Фиктивное главное".
/// </summary>
public class DummyMainTypeConfiguration : TypeConfiguration<DummyMainEntity>
{
    #region Constructors

    /// <inheritdoc/>
    public DummyMainTypeConfiguration(TypesOptions typesOptions)
        : base(typesOptions)
    {
    }
    ...
    /// <inheritdoc/>
    public sealed override void Configure(EntityTypeBuilder<DummyMainEntity> builder)
    {
        var options = TypesOptions.DummyMain;

        if (options is null)
        {
            throw new NullVariableException<DummyMainTypeConfiguration>(nameof(options));
        }
